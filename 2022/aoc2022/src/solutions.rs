...
pub fn day9() -> (i32, i32) {
    ...
    for line in pzl.lines() {
        ...
        for _ in 0..distance {
            &mut rope[0].move_toward(x_dir, y_dir);
            ...
            &mut visited_p1.insert(rope[1].coords());
            &mut visited_p2.insert(rope[9].coords());
        }
    }
    return (visited_p1.len() as i32, visited_p2.len() as i32);
}
